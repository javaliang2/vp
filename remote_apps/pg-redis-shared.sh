...
db_exec() { local d="$1"; shift
    compose_run "$d" exec -T -e PGPASSWORD="${POSTGRES_PASSWORD}" db psql -U postgres -v ON_ERROR_STOP=1 "$@"
}

db_sql() {   # DIR SQL
    compose_run "$1" exec -T -e PGPASSWORD="${POSTGRES_PASSWORD}" \
...
HBA
    ...
    if _svc_exists "$dir" "db" 2>/dev/null; then
        compose_run "$dir" exec -T -e PGPASSWORD="${POSTGRES_PASSWORD}" db \
...
EOF
    ...
    if [[ "${DEPLOY_DB:-0}" == "1" ]]; then
        ...
        if compose_run "$dir" exec -T -e PGPASSWORD="${POSTGRES_PASSWORD}" db \
    ...
    if [[ "${DEPLOY_REDIS:-0}" == "1" ]]; then
        if _svc_exists "$dir" "redis"; then
            local _redis_cli=(compose_run "$dir" exec -T redis redis-cli -h 127.0.0.1 -a "${REDIS_PASSWORD}" --no-auth-warning)
            local _orig_aofpct
...
cmd_restore() {
    ...
    if [[ "${DEPLOY_DB:-0}" == "1" && -f "$extract/postgres-all.sql.gz" ]]; then
        ...
        if gzip -dc "$extract/postgres-all.sql.gz" \
                | compose_run "$dir" exec -T -e PGPASSWORD="${POSTGRES_PASSWORD}" db psql -U postgres; then
...
cmd_status() {
    ...
    if _svc_exists "$dir" "redis"; then
        header "Redis"
        if compose_run "$dir" exec -T redis redis-cli -h 127.0.0.1 -a "${REDIS_PASSWORD}" --no-auth-warning ping 2>/dev/null | grep -q PONG; then
            log "✓ 响应正常"
            compose_run "$dir" exec -T redis redis-cli -h 127.0.0.1 -a "${REDIS_PASSWORD}" --no-auth-warning \
