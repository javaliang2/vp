...
# v4.1  单容器全打包版（修复版）
#   - 修复: WG_IP 动态注入（entrypoint 占位符替换）
#   - 修复: wp-config.php 自动生成（菜单2不再卡死）
#   - 修复: 节点删除后同步 nodes.conf
#   - 修复: 工作节点上传目录权限
#   - 主节点：build 含 wp-core/wp-content 完整镜像 → push 内网 registry
#   - 工作节点：pull 镜像 → up -d → 全层缓存刷新
#   - 首次/更新 统一走菜单 4（工作节点拉取部署）
# ============================================================
set -uo pipefail
...
# ── nginx wordpress server 块 ────────────────────────
# 当 WG_IP 为空时保留 __WG_IP__ 占位符，否则直接替换（主节点本地挂载用）
...
_write_wp_config_extra() {
    ...
    if [[ "$NODE_ROLE" == "worker" ]]; then
        DISALLOW_BLOCK="define('DISALLOW_FILE_MODS', true); // 工作节点禁止后台安装/更新"
    fi
...
# ── 主节点完整 Dockerfile（打包 wp-core + themes + plugins）──
...
RUN apk add --no-cache \
    ...
    && docker-php-ext-install -j$(nproc) \
        gd mysqli pdo_mysql zip intl exif opcache bcmath \
...
# ── 初始化 Dockerfile（主节点首次建站，不含 wp-core）──
...
# ── docker-compose.yml（主节点初始化，配置文件挂载）──
_write_init_compose() {
    local DIR="$1"
...
services:
  wordpress:
    ...
    volumes:
      # 主节点：wp 核心 + 内容 全部挂载方便后台操作
      - ./data:/var/www/html
      # nginx / php / supervisor 配置挂载（首次建站用）
...
# ── docker-compose.yml（工作节点，从仓库拉镜像）────────
...
services:
  wordpress:
    ...
    environment:
      ...
      WORDPRESS_CONFIG_EXTRA: "require_once('/etc/wordpress/wp-config-extra.php');"
    volumes:
      # uploads 单独挂卷（媒体文件走 S3/R2，本地仅临时缓冲）
      - ./data/wp-content/uploads:/var/www/html/wp-content/uploads
      # 仅挂密钥相关配置（不含 nginx/php，已打进镜像）
      - ./conf/s3-config.php:/etc/wordpress/s3-config.php:ro
      - ./conf/wp-config-extra.php:/etc/wordpress/wp-config-extra.php:ro
      - ./logs:/var/log/nginx
YAML
...
_flush_all_caches() {
    ...
    # 1. OPcache（向 php-fpm master 发 USR2 重载信号）
    ...
    && info "  OPcache 已重置" \
    || warn "  OPcache 重置失败（可忽略）"

    # 2. WordPress 对象缓存（Redis）
    dc "$DIR" exec -T wordpress \
        wp --allow-root cache flush 2>/dev/null \
    && info "  Redis 对象缓存已刷新" \
    || warn "  Redis 对象缓存刷新失败"

    # 3. WP rewrite rules
    dc "$DIR" exec -T wordpress \
        wp --allow-root rewrite flush 2>/dev/null \
    && info "  Rewrite rules 已刷新" \
    || warn "  Rewrite rules 刷新失败"

    # 4. Nginx fastcgi_cache（如果配置了）
    ...
    if [[ -n "$NGINX_CACHE_DIR" ]]; then
        dc "$DIR" exec -T wordpress \
            sh -c "rm -rf ${NGINX_CACHE_DIR}/* 2>/dev/null || true" \
        && info "  Nginx fastcgi_cache 已清理" \
        || warn "  Nginx cache 清理失败"
    fi

    # 5. Cloudflare 缓存 purge（.env 有 CF_TOKEN + CF_ZONE_ID 时执行）
    ...
    if [[ -n "$CF_TOKEN" && -n "$CF_ZONE_ID" ]]; then
        ...
        if echo "$CF_RESP" | grep -q '"success":true'; then
            info "  Cloudflare 缓存已清除"
        else
            warn "  Cloudflare purge 失败：${CF_RESP}"
        fi
    fi
...
_setup_plugins() {
    ...
    info "等待 WordPress 容器就绪（wp-cli 可用）..."
    local RETRIES=30
    ...
    # 关键修复：如果 wp-config.php 不存在，则自动创建
    if ! dc "$DIR" exec -T wordpress test -f /var/www/html/wp-config.php; then
        ...
        dc "$DIR" exec -T wordpress wp --allow-root config create \
            ...
            --extra-php <<'PHP' || {
                warn "wp-config.php 创建失败，请检查数据库连接。"
                return 1
            }
define('WP_MEMORY_LIMIT', '512M');
define('WP_MAX_MEMORY_LIMIT', '1024M');
...
EOF
    ...
    if [[ -f "$DAEMON_JSON" ]]; then
        if ! grep -q "$REGISTRY_ADDR" "$DAEMON_JSON"; then
...
JSON
    ...
    echo ""
    echo -e "  \e[36m其他节点 Docker daemon 也需添加 insecure-registries 配置\e[0m"
...
cmd_master_init() {
    ...
    case "${S3_CHOICE:-1}" in
        2) S3_PROVIDER="cloudflare"
           read -rp "R2 Endpoint URL: " S3_ENDPOINT
        ...
        3) S3_PROVIDER="other"
           ...
           [[ -z "$S3_ENDPOINT" ]] && error "必须填写 Endpoint"
    ...
    info "--- Cloudflare（可选，用于 CDN 缓存清理）---"
    read -rp "CF Zone ID（留空跳过）: " CF_ZONE_ID
    CF_ZONE_ID="${CF_ZONE_ID:-}"
    local CF_TOKEN=""
    if [[ -n "$CF_ZONE_ID" ]]; then
        read_secret "CF API Token: " CF_TOKEN
    fi
    ...
    # 创建目录结构
    mkdir -p "$DIR"/{data/wp-content/uploads,conf,logs}
...
EOF
    ...
    # 初始化 Dockerfile 和 compose（使用 entrypoint 脚本）
...
cmd_push() {
    ...
    local WP_VER
    WP_VER=$(cat "$DIR/data/wp-includes/version.php" 2>/dev/null \
        | grep wp_version | grep -oP "'\K[^']+") || WP_VER="未知"
    ...
    # 1. 同步 WordPress 核心（排除 wp-content 和 wp-config）
    ...
    # 3. 生成配置文件到 conf/（nginx 使用占位符，php 不变）
    info "生成配置文件..."
    mkdir -p "$BUILD_DIR/conf"
    _write_nginx_main_conf    "$BUILD_DIR/conf/nginx.conf"
    # 重点：空参数保留 __WG_IP__ 占位符
    ...
    # 5. 写 Dockerfile（含 wp-core + wp-content + conf 全打包，CMD 使用 entrypoint）
    ...
    # ── 清理 build context 临时目录 ──────────────────
    rm -rf "$BUILD_DIR/wp-core" "$BUILD_DIR/wp-content" "$BUILD_DIR/conf" "$BUILD_DIR/Dockerfile" "$BUILD_DIR/entrypoint.sh"
...
cmd_pull_deploy() {
    ...
    if [[ ! -f "$DIR/.env" ]]; then
        ...
        mkdir -p "$DIR"/{data/wp-content/uploads,conf,logs}
        # 确保 uploads 目录属主正确（容器内 www-data 通常为 uid 82）
        chown 82:82 "$DIR/data/wp-content/uploads" 2>/dev/null || true
...
EOF
    ...
    REGISTRY_HOST=$(env_get "$DIR/.env" "REGISTRY_HOST")
    IMAGE_TAG=$(env_get "$DIR/.env" "IMAGE_TAG")
    ...
    echo "$REG_PASS" | docker login "$REGISTRY_HOST" \
        -u "$REG_USER" --password-stdin \
    || error "仓库登录失败，请确认 Registry 地址和凭据"
...
cmd_rollback() {
    ...
    TAGS=$(curl -sf \
        -u "${REG_USER}:${REG_PASS}" \
        "http://${REGISTRY_HOST}/v2/wordpress-site/tags/list" \
        | grep -oP '"[^"]*"' | tr -d '"' | grep -v 'tags\|name' | sort -r)
    ...
    echo "$REG_PASS" | docker login "$REGISTRY_HOST" \
        -u "$REG_USER" --password-stdin &>/dev/null

    local IMAGE_FULL="${REGISTRY_HOST}/wordpress-site:${SELECTED_TAG}"
    info "拉取 ${IMAGE_FULL} ..."
    docker pull "$IMAGE_FULL" || error "拉取失败"
...
cmd_status() {
    ...
    dc "$DIR" ps
    echo ""
    local REGISTRY_HOST IMAGE_TAG WP_VER
    REGISTRY_HOST=$(env_get "$DIR/.env" "REGISTRY_HOST")
    IMAGE_TAG=$(env_get "$DIR/.env" "IMAGE_TAG")
    ...
    echo -e "  当前镜像版本:   \e[32m${IMAGE_TAG:-unknown}\e[0m"
    echo -e "  仓库地址:       \e[36m${REGISTRY_HOST:-未配置}\e[0m"
}

cmd_logs() {
    read -rp "目录 [默认: ${DEFAULT_DIR}]: " DIR; DIR="${DIR:-$DEFAULT_DIR}"
    [[ -f "$DIR/docker-compose.yml" ]] || error "未找到编排文件"
    echo "  1. 容器总日志（supervisor）"
    echo "  2. Nginx 访问日志"
    echo "  3. Nginx 错误日志"
...
cmd_nodes() {
    ...
    case "$NODE_CHOICE" in
        ...
        2)
            ...
            ;;
        3)
            [[ -f "$NODES_FILE" ]] || { warn "节点列表不存在。"; return; }
            nl -ba "$NODES_FILE"
...
interactive_menu() {
    while true; do
        echo ""
        _c "1;35" "========================================"
        _c "1;35" "  WordPress 多节点分发管理 v4.1"
