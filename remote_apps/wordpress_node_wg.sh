...
_wait_and_setup_plugin() {
    ...
    while ! wp_cli "$DIR" core is-installed &>/dev/null; do
        ...
        if [[ $RETRIES -eq 0 ]]; then
            warn "WordPress 初始化未完成，请稍后进入菜单手动安装 Offload Media 插件。"
            return 1
        fi
    done
    ...
    if [[ -n "$WP_CONFIG_FILE" ]]; then
        ...
        log "s3-config.php 已经成功挂载到 WordPress 核心配置中！"
    fi
}
...
YAML
    ...
    echo -e "内网缓存登录态共享: \e[33m${REDIS_HOST}\e[0m"
    echo -e "静态资源上云桶: \e[33ms3://${S3_BUCKET}\e[0m"
    warn "请注意：媒体分离插件正在后台静默安装，等在网页端配置好站点后即可自动生效。"
}
...
interactive_menu() {
    while true; do
        ...
        echo -e "  \e[32m5.\e[0m ▶️  启动该节点服务"
        echo -e "  \e[33m6.\e[0m 🛠️  手动触发重试安装 S3 媒体分离插件"
